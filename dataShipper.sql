...
data_shipper:begin
    ...
    select sql_no_cache radacctid into rid from radius6.radacct where acctstarttime <  (NOW() - interval purge_days day) 
    and acctstoptime is not null  order by radacctid desc limit 1;
    select rid;
    ...
    update radius6.logs set stop_time=NOW(), rows_affected=rowcount, activity_time=timediff(stop_time,start_time), 
    comment='sum rows' where log_tbl_id=last_inserted_id;
    select rowcount;
    commit ;
    -- Data Shipping
    while rowcount !=0 or rowcount > 0 do
        replace into archive6.radacct select * from radius6.radacct where radacctid <= rid  and 
        acctstoptime is not null order by radacctid asc limit 3000;
        delete from radius6.radacct where radacctid <= rid and acctstoptime is not null order by 
        radacctid asc limit 3000; -- delete 3000 rows per loop
        set rowcount = rowcount - 3000;
        update radius6.logs set stop_time=NOW(), activity_time=timediff(stop_time,start_time), 
        comment=concat(rowcount,' more rows to delete'), status='continuing' where log_tbl_id=last_inserted_id;
        select rowcount as 'after_deletion';
        if rowcount = 0 or rowcount < 0 then
            update radius6.logs set stop_time=NOW(), activity_time=timediff(stop_time,start_time), 
            status='completed', comment='Purging Completed' where log_tbl_id=last_inserted_id;
            leave data_shipper;
        end if;
    end while;
end **
